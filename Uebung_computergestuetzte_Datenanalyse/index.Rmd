...
df = data.frame(
  ID = 1:N,
  groups = sample(c("No Training", "Training"), N, replace = TRUE, prob = c(0.5, 0.5)),
  Gender = sample(c("male", "female"), N, replace = TRUE, prob = c(0.46, 0.54)),
  Relationship_status = sample(c("single", "relationship"), N, replace = TRUE, prob = c(0.39, 0.61)),
  Age = pmax(round(rnorm(N, 39, 15)), 18),
  Employed = sample(c("no", "yes"), N, replace = TRUE, prob = c(0.4, 0.6)),
  Comorbidities = sample(c("none", "F32", "F60"), N, replace = TRUE, prob = c(0.8, 0.15, 0.05)),
  Conc_med = sample(c("no", "yes"), N, replace = TRUE, prob = c(0.1, 0.9))
)

gps_items = paste0("GPS_", 1:23)
sas_items = paste0("SAS_", 1:33)
df[paste0("t0_", gps_items)] = round(rnorm(N * 23, 3, 1.5), 0)
df[paste0("t1_", gps_items)] = round(rnorm(N * 23, 3, 1.5), 0)
df[paste0("t0_", sas_items)] = round(rnorm(N * 33, 3, 1.5), 0)
df[paste0("t1_", sas_items)] = round(rnorm(N * 33, 3, 1.5), 0)

recode_items = c(5, 8, 12, 16, 18, 21, 23)
for(ch in paste0("t0_GPS_", recode_items)){
  df[[ch]] = dplyr::recode(df[[ch]], `5` = 1, `4` = 2, `3` = 3, `2` = 4, `1` = 5)
}
for(ch in paste0("t1_GPS_", recode_items)){
  df[[ch]] = dplyr::recode(df[[ch]], `5` = 1, `4` = 2, `3` = 3, `2` = 4, `1` = 5)
}

df$t0_GPS_sum = rowSums(df[paste0("t0_", gps_items)])
df$t1_GPS_sum = rowSums(df[paste0("t1_", gps_items)])
df$t0_SAS_sum = rowSums(df[paste0("t0_", sas_items)])
df$t1_SAS_sum = rowSums(df[paste0("t1_", sas_items)])

training_effect_mean = 15
training_effect_sd = 5

no_training_effect_mean = 10
no_training_effect_sd = 7

df$t1_SAS_sum[df$groups == "No Training"] <- df$t0_SAS_sum[df$groups == "No Training"] + rnorm(sum(df$groups == "No Training"), training_effect_mean, training_effect_sd)
df$t1_SAS_sum[df$groups == "Training"] <- df$t0_SAS_sum[df$groups == "Training"] + rnorm(sum(df$groups == "Training"), no_training_effect_mean, no_training_effect_sd)

df$t1_GPS_sum[df$groups == "No Training"] <- df$t0_GPS_sum[df$groups == "No Training"] + rnorm(sum(df$groups == "No Training"), training_effect_mean, training_effect_sd)
df$t1_GPS_sum[df$groups == "Training"] <- df$t0_GPS_sum[df$groups == "Training"] + rnorm(sum(df$groups == "Training"), no_training_effect_mean, no_training_effect_sd)

library(ggplot2)
p1 <- ggplot(df, aes(x = groups, y = t1_SAS_sum - t0_SAS_sum)) +
  geom_boxplot() +
  ylab("Improvement Addiction") +
  theme_minimal()

p2 <- ggplot(df, aes(x = t1_GPS_sum - t0_GPS_sum, y = t1_SAS_sum - t0_SAS_sum, color = groups)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  xlab("Improvement Procrastination") +
  ylab("Improvement Addiction") +
  theme_minimal()
...
write.csv(df, "/Users/jani/Desktop/CFH/Sommersemester 2024 Einführung in R/Rep/Uebung_computergestuetzte_Datenanalyse/laste.csv")
...
smartphone = read.csv("https://raw.githubusercontent.com/janikasaretzki/CFH_Dat/main/laste.csv")
...
# Aufgabe 1
dim(smartphone) # Datensatz mit 128 Zeilen (ProbandInnen) und 125 Spalten (Variablen)
...
# Berechnung der Summenwerte für GPS und SAS, falls noch nicht vorhanden
smartphone$t0_GPS_sum <- rowSums(select(smartphone, starts_with("t0_GPS")), na.rm = TRUE)
smartphone$t1_GPS_sum <- rowSums(select(smartphone, starts_with("t1_GPS")), na.rm = TRUE)
smartphone$GPS_diff <- smartphone$t0_GPS_sum - smartphone$t1_GPS_sum

smartphone$t0_SAS_sum <- rowSums(select(smartphone, starts_with("t0_SAS")), na.rm = TRUE)
smartphone$t1_SAS_sum <- rowSums(select(smartphone, starts_with("t1_SAS")), na.rm = TRUE)
smartphone$SAS_diff <- smartphone$t0_SAS_sum - smartphone$t1_SAS_sum

# Überprüfen, ob die neuen Spalten tatsächlich im Datensatz sind
names(smartphone)

# Speichern des gesamten Datensatzes mit allen Spalten
write.csv(smartphone, "simulated_smartphone_evaluated.csv", row.names = FALSE)
...
Im Kontext des Ihnen vorliegenden smartphone-Datensatzes sind eine Vielzahl an (statistischer) Fragestellungen möglicht. 
...
smartphone = read.csv("https://raw.githubusercontent.com/janikasaretzki/CFH_Dat/main/simulated_smartphone_evaluated.csv")
...
ggplot(data = smartphone, aes(y = t0_SAS_sum - t1_SAS_sum, x = t0_GPS_sum - t1_GPS_sum, color = groups)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") + 
...
ggplot(smartphone, aes(y = change, x = GPS_diff, colour = groups)) +
