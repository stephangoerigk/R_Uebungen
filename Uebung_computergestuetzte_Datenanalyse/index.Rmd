...
table(bipolar$Groups) # Training: 68 Proband:innen, Control: 60 Proband:innen

table(bipolar$Gender[bipolar$Groups == "EW"]) # Training: 32 weibliche, 36 männliche Proband:innen
table(bipolar$Gender[bipolar$Groups == "noEW"]) # Control: 37 weibliche, 23 männliche Proband:innen

table(bipolar$Gender) # Total: 69 weiblich, 59 männlich
...
round(mean(bipolar$Age), 2) # Total (Age): M = 40.22
round(sd(bipolar$Age), 2) # Total (Age): SD = 14.53

round(mean(bipolar$Age[bipolar$Groups == "noEW"]), 2) # Control (Age): M = 39.40
round(sd(bipolar$Age[bipolar$Groups == "noEW"]), 2) # Control (Age): SD = 15.08
range(bipolar$Age[bipolar$Groups == "noEW"])
round((sum(bipolar$Age[bipolar$Groups == "noEW"] > 65) / length(bipolar$Age[bipolar$Groups == "noEW"])) * 100, 2)
...
range(bipolar$Age[bipolar$Groups == "EW"])
round((sum(bipolar$Age[bipolar$Groups == "EW"] > 65) / length(bipolar$Age[bipolar$Groups == "noEW"])) * 100, 2)
...
In diesem Abschnitt wird das im Kapitel "Datenvorverarbeitung" (Teil 2) begonnene Projekt weitergeführt. Das Ziel ist es, die bereits teilweise in Kapitel zwei ermittelten Werte grafisch darzustellen und statistische Fragestellungen mit den Ihnen bekannten Methoden zu überprüfen. Der zur Verfügung gestellte Datensatz "bipolar" bleibt derselbe, enthält aber in der aktualisierten Version, die erneut über Github heruntergeladen werden muss, bereits die wichtigsten Werte:
...
# 1. Normalverteilung (NV) der Variable "YMRS_sum" in beiden Gruppen prüfen
shapiro.test(bipolar$YMRS_sum[bipolar$Groups == "noEW"]) # Annahme der NV ist nicht verletzt (Test n.s.)
shapiro.test(bipolar$YMRS_sum[bipolar$Groups == "EW"]) # Annahme der NV ist nicht verletzt (Test n.s.)

# 2. Varianzhomogenität prüfen
library(car)
leveneTest(YMRS_sum ~ Groups, data = bipolar) # Annahme der NV ist nicht verletzt (Test n.s.)

## T-test für unabhängige Stichproben
t.test(YMRS_sum ~ Groups, data = bipolar, var.equal = TRUE)

library(effsize)
cohen.d(YMRS_sum ~ Groups, data = bipolar)

# M sollte immer mit SD berichtet werden (M aus "t.test()"-Funktion ersichtlich)
round(sd(bipolar$YMRS_sum[bipolar$Groups == "EW"]), 2) 
round(sd(bipolar$YMRS_sum[bipolar$Groups == "noEW"]), 2)

# Das Ergebnis des unabhängigen t-Tests zeigt, dass die Symptomatik im Folgemonat, gemessen durch den YMRS-Summenwert, in der Trainingsgruppe (M = 26.85, SD = 4.53) signifikant niedriger ist als in der Gruppe ohne Training (M = 31.20, SD = 4.40; t(126) = -5.49, p < .001). Die berechnete Effektstärke (Cohen's d = -0.97, 95% CI [-1.34, -0.60]) weist nach den Kriterien von Cohen (1988) auf einen großen Effekt hin.
...
## Voraussetzungen prüfen
# 1. Normalverteilung (NV) der Variablen "YMRS_sum" und "ISEL_sum" prüfen
shapiro.test(bipolar$YMRS_sum) # Annahme der NV ist nicht verletzt (Test n.s.)
shapiro.test(bipolar$ISEL_sum) # Annahme der NV ist nicht verletzt (Test n.s.)

## Pearson-Korrelation
cor.test(bipolar$YMRS_sum, bipolar$ISEL_sum, method = "pearson") # Der Pearson-Korrelationskoeffizient zeigt, dass es einen signifikanten negativen Zusammenhang zwischen der Güte des sozialen Supportsystems (ISEL_sum) und der Symptomatik (YMRS_sum) gibt (r = -0.45, p < .001). Dies bedeutet, dass ein höherer sozialer Support mit einer geringeren Symptomatik verbunden ist. Nach Cohen (1988) handelt es sich hierbei um einen moderaten Effekt.


# Fragestellung 3
## Pearson-Korrelation (differenziert nach Gruppen)

## Voraussetzungen prüfen
# 1. Normalverteilung (NV) der Variablen in beiden Gruppen prüfen
# YMRS_sum (siehe Fragestellung 1)
shapiro.test(bipolar$YMRS_sum[bipolar$Groups == "noEW"]) # Annahme der NV ist nicht verletzt (Test n.s.)
shapiro.test(bipolar$YMRS_sum[bipolar$Groups == "EW"]) # Annahme der NV ist nicht verletzt (Test n.s.)

# ISEL_sum
shapiro.test(bipolar$ISEL_sum[bipolar$Groups == "noEW"]) # Annahme der NV ist verletzt (p = 0.039)

# Anzahl der Personen in Gruppe "no Training"
length(bipolar$Groups == "kein Training") # n = 128, d.h. nach dem zentralen Grenzwerttheorem kann trotz Verletzung der NV parametrisch getestet werden (Annahme, dass Charakteristika der NV bei N ≥ 30 gegeben sind). Alternative: Nonparametrische Verfahren

shapiro.test(bipolar$ISEL_sum[bipolar$Groups == "EW"]) # Annahme der NV ist nicht verletzt (Test n.s.)

## Pearson-Korrelation
cor.test(bipolar$ISEL_sum[bipolar$Groups == "EW"], bipolar$YMRS_sum[bipolar$Groups == "EW"], method = "pearson") # Der Pearson-Korrelationskoeffizient zeigt, dass es bei Personen, die ein Training zur Erkennung von Frühwarnzeichen erhalten haben, einen signifikanten negativen Zusammenhang zwischen der Güte des sozialen Supportsystems (ISEL_sum) und der Symptomatik (YMRS_sum) gibt (r = -0.80, p < .001). Dies bedeutet, dass ein höherer sozialer Support in dieser Gruppe mit einer geringeren Symptomatik verbunden ist. Nach Cohen (1988) handelt es sich hierbei um einen großen Effekt.

cor.test(bipolar$ISEL_sum[bipolar$Groups == "noEW"], bipolar$YMRS_sum[bipolar$Groups == "noEW"], method = "pearson") # Der Pearson-Korrelationskoeffizient zeigt, dass es bei Personen, die kein Training zur Erkennung von Frühwarnzeichen erhalten haben, keinen signifikanten Zusammenhang zwischen der Güte des sozialen Supportsystems (ISEL_sum) und der Symptomatik (YMRS_sum) gibt (r = -0.18, p = 0.164).

# Mögliche Diskussion dieser dritten Fragestellung: Zusammenfassend zeigen die Ergebnisse, dass bei Personen, die ein Training zur Erkennung von Frühwarnzeichen erhalten haben, ein starker und signifikanter negativer Zusammenhang zwischen sozialem Support und Symptomschwere besteht, während bei Personen ohne solches Training kein signifikanter Zusammenhang festgestellt wurde. Dies deutet darauf hin, dass das Training möglicherweise eine Schlüsselrolle dabei spielt, wie effektiv sozialer Support zur Reduktion der Symptomschwere beiträgt.

```

<hr> <!-- Horizontale Trennlinie -->

**Aufgabe 2:** Im Folgenden finden Sie (neben den bereits in der Lösung zu Aufgabe 1 beschriebenen Beispielen) ein Beispiel, wie Sie die aus Ihren Berechnungen gewonnenen Erkenntnisse visualisieren könnten.
...
group_colors = c("EW" = "#F8766D", "noEW" = "#00BFC4")
...
barplot_bipolar = ggplot(bipolar, aes(x = Groups, y = YMRS_sum, fill = Groups)) +
  ...
  scale_x_discrete(labels = c("noEW" = "Control", "EW" = "Training")) +  
  scale_fill_manual(values = group_colors, labels = c("Training", "Control")) +  
...
scatterplot_bipolar = ggplot(data = bipolar, aes(x = ISEL_sum, y = YMRS_sum, color = Groups)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") + 
  scale_color_manual(values = group_colors, labels = c("Training", "Control")) +  
...
ggarrange(barplot_bipolar, scatterplot_bipolar, 
          ncol = 2, nrow = 1, 
...
Willkommen im Abschnitt **Poweranalyse und Stichprobenumfangsplanung in R und RStudio**!

In diesem Kapitel der learnR-Oberfläche werden Sie Ihre Kenntnisse in der Poweranalyse und der Stichprobenumfangsplanung festigen und praktisch anwenden.

Die Poweranalyse ermöglicht es, die Wahrscheinlichkeit zu bestimmen, mit der ein statistischer Test einen tatsächlich vorhandenen Effekt entdeckt. Ein zentrales Grundkonzept dabei ist, dass die $\alpha$-Fehler-Wahrscheinlichkeit, die Teststärke (Power, 1-$\beta$), die Effektgröße und die Stichprobengröße funktional miteinander verknüpft sind, sodass bei der Vorgabe von drei dieser Größen die jeweils vierte eindeutig berechnet werden kann.

Bei der Planung des Stichprobenumfangs gilt es, eine Balance zwischen zwei Prinzipien zu finden: **Präzisionssteigerung** und **Erhebungsökonomie**. Mit zunehmender Stichprobengröße können Effekte präziser geschätzt und zuverlässiger aufgedeckt werden (Präzisionssteigerung). Gleichzeitig soll die Stichprobe so bemessen sein, dass Effekte, die als praktisch bedeutsam erachtet werden, signifikant werden, während triviale Effekte nicht unnötig als signifikant erkannt werden (Erhebungsökonomie).

In den kommenden Aufgaben werden Sie Ihr Wissen über die verschiedenen Ansätze zur Poweranalyse und zur Berechnung des optimalen Stichprobenumfangs anwenden und vertiefen. Sie werden praxisnahe Szenarien bearbeiten, sodass Sie die erlernten Prinzipien auch in Ihren eigenen Projekten anwenden können.

<hr> <!-- Horizontale Trennlinie -->

**Aufgabe 1:** In einer klinischen Studie zur Wirksamkeit eines neuen Medikaments wurde die Korrelation zwischen der Dosierung (*r* = .25) und der Verbesserung der Symptome bei einer Stichprobengröße von *N* = 40 untersucht. Das Ergebnis zeigte mit einem *p*-Wert von .120, dass die Korrelation nicht signifikant ist. Nun möchten Sie ermitteln, welche Stichprobengröße erforderlich ist, um eine signifikante Korrelation bei einem Signifikanzniveau von $\alpha$ = .05 zu erreichen.
```{r Poweranalyse1, exercise=TRUE}


```

```{r Poweranalyse1-solution}

library(pwr)
pwr.r.test(r = .25, sig.level = .05, power = .80) 

# Um eine signifikante Korrelation von r = .25 bei einem Signifikanzniveau von α = .05 mit einer Teststärke (Power) von .80 zu erreichen, wird eine Stichprobengröße N = 122 benötigt.

```

<hr> <!-- Horizontale Trennlinie -->

**Aufgabe 2:** In einer klinischen Studie zur Bewertung der Wirksamkeit eines neuen Antidepressivums im Vergleich zu einem Placebo möchten die Forscher:innen sicherstellen, dass auch kleinere, aber klinisch relevante Effekte entdeckt werden. Aufgrund der strengen regulatorischen Anforderungen haben sie ein Signifikanzniveau von $\alpha$ = .01 festgelegt. Die Forscher:innen erwarten eine Effektgröße von *d* = .30. Um sicherzustellen, dass die Studie eine ausreichende Power von .80 erreicht, soll die erforderliche Stichprobengröße pro Gruppe berechnet werden.
```{r Poweranalyse2, exercise=TRUE}

```

```{r Poweranalyse2-solution}

library(pwr)
pwr.t.test(d = 0.30, sig.level = 0.01, power = 0.80, type = "two.sample")

# Um eine Effektgröße von d = .30 bei einem strengen Signifikanzniveau von α = .01 und einer Power von .85 zu entdecken, werden in jeder Gruppe etwa N = 261 Proband:innen benötigt. Diese größere Stichprobe ist notwendig, um auch kleine, aber klinisch relevante Unterschiede zwischen dem neuen Antidepressivum und dem Placebo zuverlässig feststellen zu können.
