...
#                                         round(rnorm(N, mean=5, sd=4)),  
#                                         round(rnorm(N, mean=2, sd=4)))   
# 
# df$change <- df$t0_SAS_sum - df$t1_SAS_sum
# 
# df$GPS_diff <- ifelse(df$groups == "Training",
#                       round(rnorm_pre(x = df$change, mu = 20, sd = 12, r = 0.8, empirical = TRUE)),  
#                       round(rnorm_pre(x = df$change, mu = 10, sd = 12, r = -0.4, empirical = TRUE)))  
# 
...
```{r Simulation JS ohne Ausreißer, include = FALSE}
...
df <- df[!(df$change < -8 & df$groups == "Training"), ]
